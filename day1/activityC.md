...
The basis of this algorithm is the quartet graph; in building the graph, TREE-QMC additionally weights quartets based on their known closeness to a given subproblem (i.e., branch in the species tree) and does so in such a way that the expected frequencies under MSC are not perturbed.
